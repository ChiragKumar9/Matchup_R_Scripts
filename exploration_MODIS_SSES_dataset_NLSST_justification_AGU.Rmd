...
## SST as a Separator
```{r band_diff_temp_deficit_context_SST}
...
```{r band_diff_temp_deficit_context_residual_qsst0_only}
...
uuu <- data.frame(temp_context = as.factor(resid_context), temp_deficit = temp_deficit, band.diff = zz1$band.diff)
...
```{r band_diff_temp_deficit_context_residual_all_qualities}
...
## SST as a Separator
```{r band_diff_product_temp_deficit_context_sst}
...
ggplot2::ggplot(data = uuu, aes(x = product, y = temp_deficit)) +
  geom_bin2d(bins = 50) +
  scale_fill_gradientn(colours = ccc) + 
  geom_smooth(method = loess, se = FALSE, span = 0.5) +
  facet_wrap(~temp_context, nrow = 3, ncol = 1) +
  labs(x = '(T_11 - T_12) * ref_SST', y = 'Temperature Deficit')
  #ggsave('band_diff_predictor_of_temp_deficit_different_in_situ_windows.pdf')
...
## Residual as a Separator
```{r band_diff_product_temp_deficit_context_residual_qsst0_only}
...
uuu <- data.frame(temp_context = as.factor(resid_context), temp_deficit = temp_deficit, product = (zz1$band.diff * zz1$ref_SST))
...
Same plot as above but qsst == 0, 1, or 2.
```{r band_diff_product_temp_deficit_context_residual_all_qualities}
