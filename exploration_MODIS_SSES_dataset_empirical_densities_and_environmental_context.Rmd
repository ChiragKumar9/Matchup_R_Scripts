...
yy <- lubridate::year(orig_filtered$buoy.timedate)     # Year
mm <- lubridate::month(orig_filtered$buoy.timedate)    # Month
...
ggplot2::ggplot(data = orig, aes(x = buoy.sst, y = SST.resid.SMB + 0.17,
  ...
  geom_bin2d(bins = 100) + 
  #scale_fill_gradientn(colours = ccc)cat('\nSummary Statistics for Quality = 0\n')
summary(orig$SST.resid.SMB[orig$qsst == 0])
cat('\n Standard Deviation for Quality = 0: ')
sd(orig$SST.resid.SMB[orig$qsst == 0]) +
...
prod_buoy_SST_band_diff <- orig$buoy.sst * orig$band_diff
