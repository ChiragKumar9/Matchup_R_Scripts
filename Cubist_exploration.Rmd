...
regression_error_stats <- function(predicted, observed) {
  ...
  RMSE <- caret::RMSE(Cubist_rules_train_vals, SSES_df_train$SST.resid.SMB)
  MAE <- hydroGOF::mae(Cubist_rules_train_vals, SSES_df_train$SST.resid.SMB)
  d_willmott <- hydroGOF::d(Cubist_rules_train_vals, SSES_df_train$SST.resid.SMB)
  NSE_efficiency <- hydroGOF::NSE(Cubist_rules_train_vals, SSES_df_train$SST.resid.SMB)
  ratio_sds <- hydroGOF::rSD(Cubist_rules_train_vals, SSES_df_train$SST.resid.SMB)
...
orig <- dplyr::tbl_df(AQUA) %>%
  ...
  dplyr::select(T_11, T_12, band.diff, ref_SST, x2, x3, satz, abs_satz, lon, lat, month, sd11, sd12, range11, range12, diff.med.min11,
                diff.med.min12, qsst, buoy.sst, cell5deg, cen.sst, td_estimate, td_actual, SST.resid.SMB)

# Subset the data such that we are looking at retrievals only within the Nino 3.4 zone
orig <- orig %>% 
...
```{r correlogram_of_specific_features_and_residual, echo=TRUE}
# Take the correlation matrix
...
SSES_df_train <- SSES_df_temp_N34[index, ]
SSES_df_test <- SSES_df_temp_N34[-index, ]
```

Statistics of the residuals (i.e., what Cubist will try to predict)
```{r statistics_of_residuals, echo=TRUE}
summary(SSES_df_train$SST.resid.SMB)
cat('SD of train residuals: ')
sd(SSES_df_train$SST.resid.SMB)
```


```{r Cubist, echo=TRUE}
Cubist_rules_expl <- Cubist::cubist(x = SSES_df_train[, c('td_resid',
                                                          'x2',
                                                          #'abs_satz'
                                                          'ref_SST',
                                                          #'lat',
...
```{r Cubist_tree_print_out, echo=TRUE}
summary(Cubist_rules_expl)
```

```{r diagnostic_plots, echo=TRUE}
dotplot(Cubist_rules_expl, what = 'splits')
dotplot(Cubist_rules_expl, what = 'coefs')
```


```{r predicted_and_actual_residuals, echo=TRUE}
...
yyy <- data.frame(Y = Cubist_rules_test_vals, X = SSES_df_test$SST.resid.SMB)
...
```{r statistics_for_model_evaluation_train_set, echo=TRUE}
# 1-D Statistics
...
train_model_resids <- Cubist_rules_train_vals - SSES_df_train$SST.resid.SMB
summary(train_model_resids)
sd(train_model_resids)

# Measures of error/accuracy
regression_error_stats(Cubist_rules_train_vals, SSES_df_train$SST.resid.SMB)

```


```{r statistics_for_model_evaluation_test_set, echo=TRUE}
# 1-D Statistics
# Stats for actual residuals
summary(SSES_df_test$SST.resid.SMB)
sd(SSES_df_test$SST.resid.SMB)

# Just of Predictions
summary(Cubist_rules_test_vals)
sd(Cubist_rules_test_vals)

test_model_resids <- Cubist_rules_test_vals - SSES_df_test$SST.resid.SMB
...
# Measures of error/accuracy
regression_error_stats(Cubist_rules_test_vals, SSES_df_test$SST.resid.SMB)
...
for (variable_num in 1:nrow(MAE_without)) {
  ...
  ppp <- SSES_df_train[, c(variables_temp, 'SST.resid.SMB')]
  ...
  Cubist_predictions_test_temp <- stats::predict(object = Cubist_var_imp, na.action = na.pass, neighbors = 8, newdata = SSES_df_test)
  
  # Now compute RMSE
  MAE_temp <- hydroGOF::mae(Cubist_predictions_test_temp, SSES_df_test$SST.resid.SMB)
  
  MAE_without[variable_num, 'MAE'] <- MAE_temp

}

MAE_without$MAE_incr <- MAE_without$MAE - 0.187825
