...
orig <- dplyr::tbl_df(AQUA) %>%
  ...
  dplyr::select(T_11, T_12, band.diff, ref_SST, x2, x3, satz, lon, lat, month, sd11, sd12, range11, range12, diff.med.min11, diff.med.min12,
    qsst, buoy.sst, cell5deg, cen.sst, td_estimate, SST.resid.SMB)
...
orig$td_resid <- stats::predict(td_resid, newdata = orig$x2)
...
# Take the correlation matrix
eee <- orig[names(orig) != 'buoy.sst']
cors_mat_o <- as.matrix(cor(eee))
cors_df_o <- as.matrix(cor(eee))
