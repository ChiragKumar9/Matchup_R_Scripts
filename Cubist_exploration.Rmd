...
orig <- dplyr::tbl_df(AQUA) %>%
  ...
  dplyr::select(T_11, T_12, band.diff, ref_SST, x2, x3, satz, abs_satz, lon, lat, month, sd11, sd12, range11, range12, diff.med.min11, diff.med.min12,
...
SSES_df_temp_N34 <- dplyr::tbl_df(orig2) %>%
  dplyr::select(td_resid,
                x2,
                satz,
                abs_satz,
...
cat('sd of train SST residuals: ')
sd(SSES_df_temp_train$SST.resid.SMB)

Cubist_rules_expl <- Cubist::cubist(x = SSES_df_temp_train[, c('td_resid', 'x2', 'satz', 'ref_SST', 'lat', 'month', 'sd11')], y = SSES_df_temp_train$SST.resid.SMB,
                                    committees = 1,
...
Cubist_rules_test_vals <- stats::predict(object = Cubist_rules_expl, newdata = SSES_df_temp_test, na.action = na.pass, neighbors = N_correction_neighbors)
