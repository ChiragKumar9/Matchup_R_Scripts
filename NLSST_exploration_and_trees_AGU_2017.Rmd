...
```{r evaluate_regression_statistically, echo=TRUE}
...
medians <- tapply(orig_train$SST.resid.SMB, ctree_big_4_var_tree_train_nodes, median)
IQRs <- tapply(orig_train$SST.resid.SMB, ctree_big_4_var_tree_train_nodes, IQR)
lengths <- tapply(orig_train$SST.resid.SMB, ctree_big_4_var_tree_train_nodes, length)

hhh <- data.frame(SST.resid.SMB = orig_train$SST.resid.SMB, node = ctree_big_4_var_tree_train_nodes)

ggplot2::ggplot(data = hhh) +
...
```{r M5P, echo=TRUE}
M5P_tree <- RWeka::M5P(formula = SST.resid.SMB ~ td_resid + x2 + sec_satz + sd11,
                       data = orig_train)
...
## Use the tree to predict
### Train set
M5P_tree_train_vals <- stats::predict(object = M5P_tree, newdata = orig_train, na.action = na.pass)

### Test set
M5Pr_tree_test_vals <- stats::predict(object = M5P_tree, newdata = orig_test, na.action = na.pass)
