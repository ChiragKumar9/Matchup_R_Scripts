...
rasterVis::gplot(cell5deg) + 
  ggplot2::geom_raster(aes(fill = (value))) + scale_fill_gradientn(colors = palette) +
  labs(x = '(T_11 - T_12) * Reference SST', y = 'Temperature Deficit', fill = 'Fraction of\nGood Retrievals') +
  ggplot2::theme_bw()
```

```{r temp_def_lm, echo=TRUE}

td_fit <- lm(temp_def ~ orig$x2)
...
Forests. All trees receive the same 4 splitting variables: x2, x3, temperature deficit estimate, 
and sd_11.
...
# Build the tree
rpart_4_var_tree <- rpart::rpart(formula = class_resid ~ ref_SST_minus_T_11 + x2 + x3 + sd11,
...
# Build the tree
ctree_4_var_tree <- partykit::ctree(formula = class_resid ~ ref_SST_minus_T_11 + x2 + x3 + sd11,
                                    data = orig_train_smote,
                                    minbucket = 20,
                                    mincrit = 0.95,
                                    maxdepth = 5)
...
# Build the tree
ctree_4_var_tree_big <- partykit::ctree(formula = class_resid ~ ref_SST_minus_T_11 + x2 + x3 + sd11,
                                    data = orig_train_smote,
                                    minbucket = 20,
                                    mincrit = 0.95)
# Plot the tree
...
# Build the tree
rand_for_4_var <- randomForest::randomForest(formula = class_resid ~ ref_SST_minus_T_11 + x2 + x3 + sd11,
                                               data = orig_train_smote,
                                               nodesize = 20,
                                               ntree = 1000,
                                               mtry = 3)
...
### Test set
rand_4_var_test_predictions <- stats::predict(object = rand_for_4_var, newdata = orig_test, type = 'class', na.action = na.pass)
...
### Test set
rand_for_4_var_test_cm <- caret::confusionMatrix(data = rand_4_var_test_predictions, reference = orig_test$class_resid)

rand_for_4_var_test_cm
```

### C5.0
```{r C50_4_var_tree_classification, echo=TRUE}
# Build the tree
C50_4_var_tree <- C50::C5.0(class_resid ~ ref_SST_minus_T_11 + x2 + x3 + sd11, data = orig_train_smote)

# Plot the tree
plot(C50_4_var_tree)

plot(partykit::as.party(C50_4_var_tree))

# Evaluate the tree
## Use the tree to predict
### Train set
C50_4_var_tree_train_predictions <- stats::predict(object = C50_4_var_tree, newdata = orig_train_smote, na.action = na.pass)

### Test set
C50_4_var_tree_test_predictions <- stats::predict(object = C50_4_var_tree, newdata = orig_test, na.action = na.pass)
...
```{r C50_4_var_tree_regression, echo=TRUE}
# Build the tree
C50_4_var_tree <- C50::C5.0(SST.resid.SMB ~ ref_SST_minus_T_11 + x2 + x3 + sd11, data = orig_train_smote)
