...
uuu.kde <- kde2d(uuu[,1], uuu[,2], n = 400)
dx <- diff(uuu.kde$x[1:2])  # lifted from emdbook::HPDregionplot()
dy <- diff(uuu.kde$y[1:2])
sz <- sort(uuu.kde$z)
c1 <- cumsum(sz) * dx * dy
...
# plot:
dimnames(uuu.kde$z) <- list(uuu.kde$x, uuu.kde$y)
dc <- melt(uuu.kde$z)
dc$prob <- approx(sz, 1 - c1, dc$value)$y
p <- ggplot(dc, aes(x = Var1, y = Var2)) +
  geom_contour(aes(z = prob, color = resid_context), breaks = prob) +
  #geom_point(aes(x = X1, y = X2, color = resid_context), data = uuu, alpha = 0.1, size = 1) +
  labs(x = '(T_11 - T_12) * Reference SST', y = 'Temperature Deficit', color = 'Residual Range')
print(p)
```
