...
M5P_tree <- Cubist::cubist(x = orig_train[, c('td_resid', 'x2', 'sec_satz', 'sd11', 'lat', 'month')], y = orig_train$SST.resid.SMB,
                           committees = 1,
                           control = Cubist::cubistControl(rules = 120)) # 46 rules without latitude and month to not exceed minsplit rules used above

capture.output(summary(M5P_tree), file = 'M5P_tree_rules_and_stats_rules=120_with_lat_and_month.txt')
