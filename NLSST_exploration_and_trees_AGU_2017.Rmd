...
rasterVis::gplot(cell5deg) + 
  ...
  labs(x = '(T_11 - T_12) * Reference SST', y = 'Actual TD - Predicted TD', fill = 'Fraction of\nGood Retrievals') +
  ggplot2::theme_bw()
```
...
# Build the tree
rpart_4_var_tree <- rpart::rpart(formula = class_resid ~ ref_SST_minus_T_11 + x2 + sec_satz + sd11,
                                 data = orig_train_smote,
                                 control = rpart.control(minbucket = 82, maxdepth = 7, xval = 3, cp = 0.01))
# Plot the tree
...
# Build the tree
ctree_4_var_tree <- partykit::ctree(formula = class_resid ~ ref_SST_minus_T_11 + x2 + sec_satz + sd11,
                                    data = orig_train_smote,
                                    minbucket = 82,
                                    mincrit = 0.95,
                                    maxdepth = 5)
...
# Build the tree
ctree_4_var_tree_big <- partykit::ctree(formula = class_resid ~ ref_SST_minus_T_11 + x2 + sec_satz + sd11,
                                    data = orig_train_smote,
                                    minbucket = 82,
                                    mincrit = 0.95,
                                    maxdepth = 7)
...
# Build the tree
rand_for_4_var <- randomForest::randomForest(formula = class_resid ~ ref_SST_minus_T_11 + x2 + sec_satz + sd11,
                                               data = orig_train_smote,
                                               nodesize = 82,
                                               ntree = 750,
                                               mtry = 2)
