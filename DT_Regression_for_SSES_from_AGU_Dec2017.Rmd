...
blues_palette <- blues_palette[3:9]
blues_palette <- colorRampPalette(blues_palette)(17)
reds_palette <- RColorBrewer::brewer.pal(9, 'Reds')
reds_palette <- reds_palette[3:9]
reds_palette <- colorRampPalette(reds_palette)(17)
...
p <- rasterVis::levelplot(grid5deg, margin = FALSE,
  at = c(-Inf, seq(-4, 4, 0.25), Inf), # For med
...
breaks <- c(0, 6.25, 12.5, 25, 50, 100, 250, 500, 1000, Inf)
breaks <- breaks * 20
...
rasterVis::gplot(cell5deg) + 
  ...
  ggsave('x2_vs_TD_raster_median.pdf', device = 'pdf', width = 8, height = 6, units = 'in') +
  ggsave('x2_vs_TD_raster_median.png', device = 'png', width = 8, height = 6, units = 'in')
```
...
breaks_IQR <- c(seq(from = 0, to = 1.5, by = 0.15), Inf)
...
rasterVis::gplot(cell5deg) + 
  ...
  labs(x = 'Scaled Channel Difference (deg C)', y = 'Temperature Deficit (deg C)', fill = 'IQR of\nResiduals (deg C)') +
  coord_fixed(ratio = 2.4) +
  xlim(-2, 122) +
  ylim(-.1, 22) +
  theme(legend.key = element_blank(), legend.key.size = unit(12, "point")) +
  ggsave('x2_vs_TD_raster_IQR.pdf', device = 'pdf', width = 8, height = 6, units = 'in') +
  ggsave('x2_vs_TD_raster_IQR.png', device = 'png', width = 8, height = 6, units = 'in')
```
...
capture.output(summary(Cubist_rules), file = 'Cubist_rules=28_committees=1_with_lat_and_month.txt')
...
# Rule 11
orig_b$rule_numb[is.na(orig_b$rule_numb) & orig_b$x2 > 45.74037 & ori  Rule 1: [990 cases, mean 3169.0, range 9 to 10649, est err 6106.9]

    if
	x1 <= 11
    then
	outcome = -33383 + 5852 x1

  Rule 2: [2160 cases, mean 30001.0, range 9 to 110593, est err 4059.8]

    if
	x1 <= 24
    then
	outcome = -77349 + 7093 x1g_b$satz > 48.5194] <- 11
...
```{r tree_reconstruction_of_rules}
