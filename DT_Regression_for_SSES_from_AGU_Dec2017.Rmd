...
breaks_med <- c(-Inf, -6, -5.5, -5, -4.5, -4, -3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, Inf)
...
Cubist_rules <- Cubist::cubist(x = orig_train[, c('td_resid', 'x2', 'sec_satz', 'sd11', 'lat', 'month')], y = orig_train$SST.resid.SMB,
                           committees = 1,
                           control = Cubist::cubistControl(rules = 130)) # 46 rules without latitude and month to not exceed minsplit rules used above

capture.output(summary(Cubist_rules), file = 'Cubist_rules_and_stats_rules=130_with_lat_and_month.txt')
