...
orig <- dplyr::tbl_df(AQUA) %>%
  ...
  dplyr::select(T_11, T_12, band.diff, ref_SST, x2, x3, satz, lon, lat, sd11, sd12, range11, range12, diff.med.min11, diff.med.min12,
    qsst, buoy.sst, cell5deg, cen.sst, pseudo.resid.SMR, buoy.timedate, SST.resid.SMB)
...
raster::xyFromCell(object =  grid5deg, cell = (72 * 36), spatial = FALSE)
...
palette <- c(blues_palette, reds_palette)

p <- rasterVis::levelplot(grid5deg, margin = FALSE,
  at = c(-Inf, seq(-2, 2, 0.25), Inf), # For med
...
orig_out <- dplyr::tbl_df(orig) %>%
  dplyr::filter(pseudo.resid.SMR > cutoff)
...
reds_palette <- RColorBrewer::brewer.pal(9, 'Reds')
reds_palette <- colorRampPalette(reds_palette)(7)

palette_test <- c((blues_palette), (reds_palette))

breaks_med <- c(-Inf, -6, -5.5, -5, -4.5, -4, -3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5, 3, Inf)
...
Cubist_rules <- Cubist::cubist(x = orig_train[, c('lat', 'month', 'sd11', 'td_resid', 'x2', 'satz', 'ref_SST')], y = orig_train$SST.resid.SMB,
                           ...
                           control = Cubist::cubistControl(rules = 40,
                                                           unbiased = FALSE,
                                                           extrapolation = 100,
                                                           label = 'SST Residual',
                                                           seed = 108))

capture.output(summary(Cubist_rules), file = 'Cubist_rules=40_committees=1_with_lat_and_month.txt')
...
ggplot2::ggplot(data = yyy, aes(X, Y, fill = cut(..count.., c(0, 10, 25, 50, 100, 500, 1000, 2500, 5000, 10000, Inf)))) +
  ...
  labs(x = 'Actual Residual', y = 'Predicted Residual')
...
for (N in 1:nrow(ctrl_params_performance)) {
  ...
  if (!too_small) {
    ...
    if (min(node_sizes) >= 100) {
      ...
      cat('Committees:', committees_specific, 'out of 2; Rules:', rules_specific, 'out of 100; RMSE Test:', RMSE_test_cubist, '\n')
...
# Rule 1
orig_b$rule_numb[orig_b$td_resid > 0.5109711 & orig_b$satz > -0.2305 & orig_b$satz <= 49.0389 & orig_b$ref_SST > 24.63] <- 1

# Rule 2
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$td_resid > 0.5109711 & orig_b$satz > -45.5 & orig_b$satz <= -0.2305 & orig_b$ref_SST > 24.61] <- 2

# Rule 3
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$td_resid > 0.5109711 & orig_b$satz > -45.5 & orig_b$satz <= -0.2305 & orig_b$ref_SST <= 24.61] <- 3

# Rule 4
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$td_resid > 0.5109711 & orig_b$satz > -0.2305 & orig_b$satz <= 49.0389 & orig_b$ref_SST <= 24.63] <- 4

# Rule 5
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= 22.77 & orig_b$td_resid > 0.5109711 & orig_b$x2 <= 45.23094 & satz > 49.0389 & orig_b$ref_SST > 24.9] <- 5

# Rule 6
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$td_resid > -0.6436465 & orig_b$x2 <= 58.57362 & orig_b$satz > 1.9999 & orig_b$satz <= 40.15 & orig_b$ref_SST > 24.74] <- 6

# Rule 7
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$sd11 > 0.3425 & orig_b$x2 <= 56.5806 & orig_b$satz > -37.2888 & orig_b$satz <= 1.9999 & orig_b$ref_SST > 24.57] <- 7

# Rule 8
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= 22.77 & orig_b$td_resid > 0.5109711 & orig_b$x2 <= 16.40869 & orig_b$satz > 49.0389] <- 8

# Rule 9
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > 22.77 & orig_b$td_resid > 0.5109711 & orig_b$x2 <= 23.78199 & orig_b$satz > 49.0389] <- 9

# Rule 10
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$x2 > 56.5806 & orig_b$satz > -37.2888 & orig_b$satz <= 1.9999 & orig_b$ref_SST > 24.57] <- 10

# Rule 11
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$x2 > 58.57362 & orig_b$satz > 1.9999 & orig_b$satz <= 40.15 & orig_b$ref_SST > 24.74] <- 11

# Rule 12
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= 26.09 & orig_b$x2 <= 49.05643 & orig_b$satz <= -45.5 & orig_b$ref_SST > 24.84] <- 12

# Rule 13
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > 26.09 & orig_b$td_resid > 0.5109711 & orig_b$x2 <= 18.22698 & orig_b$satz <= -45.5] <- 13

# Rule 14
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= 22.77 & orig_b$sd11 <= 0.0667 & orig_b$td_resid > 0.5109711 & orig_b$satz > 49.0389] <- 14

# Rule 15
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$x2 > 45.23094 & orig_b$satz > 49.0389] <- 15

# Rule 16
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$x2 > 58.58734 & orig_b$satz > 40.15] <- 16

# Rule 17
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$x2 > 49.05643 & orig_b$satz <= -45.5] <- 17

# Rule 18
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > 22.77 & orig_b$x2 > 23.78199 & orig_b$satz > 49.0389] <- 18

# Rule 19
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= 22.77 & orig_b$x2 > 16.40869 & orig_b$satz > 49.0389 & orig_b$ref_SST <= 24.9] <- 19

# Rule 20
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$x2 <= 58.58734 & orig_b$satz > 40.15 & orig_b$satz <= 53.5194 & orig_b$ref_SST > 24.74] <- 20

# Rule 21
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > 26.09 & orig_b$x2 > 18.22698 & orig_b$satz <= -45.5] <- 21

# Rule 22
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= 26.09 & orig_b$x2 <= 49.05643 & orig_b$satz <= -45.5 & orig_b$ref_SST <= 24.84] <- 22

# Rule 23
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= 16.48 & orig_b$sd11 <= 0.3425 & orig_b$td_resid <= 0.5109711 & orig_b$x2 <= 56.5806 & orig_b$satz > -37.2888 & orig_b$satz <= 1.9999 & orig_b$ref_SST > 24.57] <- 23

# Rule 24
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > 16.48 & orig_b$sd11 <= 0.3425 & orig_b$td_resid <= 0.5109711 & orig_b$x2 <= 56.5806 & orig_b$satz > -37.2888 & orig_b$satz <= 1.9999 & orig_b$ref_SST > 24.57] <- 24

# Rule 25
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$sd11 > 0.0842 & orig_b$td_resid <= 0.5109711 & orig_b$satz > 1.9999 & orig_b$satz <= 43.85 & orig_b$ref_SST <= 24.74] <- 25

# Rule 26
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > 19.06 & orig_b$td_resid <= 0.5109711 & orig_b$x2 <= 58.57362 & orig_b$satz > 1.9999 & orig_b$satz <= 40.15 & orig_b$ref_SST > 24.74] <- 26

# Rule 27
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= 19.06 & orig_b$td_resid <= -0.6436465 & orig_b$x2 <= 58.57362 & orig_b$satz > 1.9999 & orig_b$satz <= 40.15 & orig_b$ref_SST > 24.74] <- 27

# Rule 28
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= -18.62 & orig_b$td_resid <= 0.5109711 & orig_b$satz <= 1.9999 & orig_b$ref_SST <= 24.57] <- 28

# Rule 29
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$td_resid <= 0.5109711 & orig_b$x2 <= 53.19003 & orig_b$satz <= -37.2888 & orig_b$ref_SST > 24.57] <- 29

# Rule 30
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$td_resid <= 0.5109711 & orig_b$x2 > 53.19003 & orig_b$satz <= -37.2888] <- 30

# Rule 31
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= -13.19 & orig_b$sd11 <= 0.0842 & orig_b$td_resid <= 0.5109711 & orig_b$satz > 1.9999 & orig_b$satz <= 43.85] <- 31

# Rule 32
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > -18.62 & orig_b$td_resid <= 0.5109711 & orig_b$satz > -50.2305 & orig_b$satz <= 1.9999 & orig_b$ref_SST <= 24.57] <- 32

# Rule 33
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > 22.27 & orig_b$td_resid <= 0.5109711 & orig_b$x2 <= 58.58734 & orig_b$satz > 40.15 & orig_bref_SST > 24.74] <- 33

# Rule 34
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > -13.19 & orig_b$sd11 <= 0.0842 & orig_b$td_resid <= 0.5109711 & orig_b$satz > 1.9999 & orig_b$satz <= 43.85 & orig_b$ref_SST <= 24.74] <- 34

# Rule 35
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat <= 32.29 & orig_b$td_resid <= 0.5109711 & orig_b$satz > 43.85 & orig_b$ref_SST <= 24.74] <- 35

# Rule 36
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$td_resid <= 0.5109711 & orig_b$x2 <= 58.58734 & orig_b$satz > 53.5194 & orig_b$ref_SST > 24.74] <- 36

# Rule 37
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > -18.62 & orig_b$lat <= 31.97 & orig_b$td_resid <= 0.5109711 & orig_b$x2 <= 32.9205 & orig_b$satz <= -50.2305] <- 37

# Rule 38
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > 32.29 & orig_b$td_resid <= 0.5109711 & orig_b$satz > 43.85 & orig_b$ref_SST <= 24.74] <- 38

# Rule 39
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$lat > 31.97 & orig_b$td_resid <= 0.5109711 & orig_bx2 <= 32.9205 & orig_b$satz <= -50.2305] <- 39

# Rule 40
orig_b$rule_numb[orig_b$rule_numb <- NA & orig_b$td_resid <= 0.5109711 & orig_b$x2 > 32.9205 & orig_b$satz <= -50.2305 & orig_b$ref_SST <= 24.57] <- 40
...
# 0.380 is the RMSE on the test set with ALL variables (from above)
RMSE_without$RMSE_incr <- RMSE_without$RMSE - 0.380
...
ggplot2::ggplot(data = RMSE_without, aes(x = variable_names_long, y = RMSE_incr)) +
  ...
  labs(x = 'Feature', y = 'Increase in RMSE')
