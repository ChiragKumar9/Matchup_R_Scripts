...
```{r rpart_binary_class_testing_rebalancing, echo=TRUE}
...
balanced_tree <- rpart::rpart(formula = class_resid ~ T_11 + T_12 + band.diff + ref_SST + ref_SST_minus_T_11 + ref_SST_minus_T_12 + x2 +
    abs(satz) + range11 + sd11 + range12 + sd12 + x3, 
                           data = orig_train_smote,
                           minbucket = 100,
                           xval = 5)

plot(as.party(balanced_tree))
...
unbalanced_tree <- rpart::rpart(formula = class_resid ~ T_11 + T_12 + band.diff + ref_SST + ref_SST_minus_T_11 + ref_SST_minus_T_12 + x2 +
    abs(satz) + range11 + sd11 + range12 + sd12 + x3, 
                           data = orig_train2,
                           minbucket = 100,
                           xval = 5)

plot(as.party(unbalanced_tree))
```
