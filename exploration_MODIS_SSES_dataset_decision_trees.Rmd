...
# Make classes
class_resid <- cut(orig$SST.resid.SMB, breaks = c(min(orig$SST.resid.SMB), -.4, (max(orig$SST.resid.SMB + 0.0001))),
...
# Evaluate the decision tree's accuracy on validation set
ctree_tree_predictions <- predict(ctree_tree, data = orig_test, type = 'class', na.action = na.pass)
...
rand_for <- randomForest::randomForest(formula = class_resid ~ T_11 + T_12 + band.diff + ref_SST + (ref_SST - T_11) + (ref_SST - T_12) + x2 +
    abs(satz) + range11 + sd11 + range12 + sd12 + x3,
                                       data = orig_train,
                                       ntree = 500, # how many trees should be in the firest we grow
...
# Evaluate the decision tree's accuracy on validation set
rand_for_predictions <- predict(rand_for, data = orig_test, type = 'class', na.action = na.pass)
