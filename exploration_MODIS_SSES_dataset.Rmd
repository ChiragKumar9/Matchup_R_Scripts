...
```{r}
in_situ_intervals <- cut(orig$buoy.sst, )
```
...
ggplot2::ggplot(data = orig) + 
  geom_density(aes(x = SST.resid.SMB + 0.17, fill = as.factor(qsst)), alpha = 0.2, color = NA) +
  geom_rug()
```
...
cutoff_val <- 0
qqq <- data.frame(condition = ifelse(orig$band.diff > cutoff_val, ifelse(orig$band.diff > 3, 2, 1), 0), resid = orig$SST.resid.SMB + 0.17)
qqq$condition <- factor(qqq$condition, levels = c(0, 1, 2), labels = c('(-Inf, 0]', '(0, 2]', '(2, Inf)'))
