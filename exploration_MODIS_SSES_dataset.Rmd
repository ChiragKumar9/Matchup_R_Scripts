...
p <- rasterVis::levelplot(grid5deg, margin = FALSE,
  ...
  at = c(1, 250, 500, 1000, 2000, 5000, 10000, Inf), # For N
  col.regions = (RColorBrewer::brewer.pal(10, 'BuGn')))
...
this places, it is important to consider, from a purely statistical perspective, they have very few retrievals in the raster cells.
...
p <- rasterVis::levelplot(grid5deg, margin = FALSE,
  ...
  at = seq(0, 0.8, 0.1), # For IQR
  col.regions = rev(RColorBrewer::brewer.pal(length(c(-Inf, seq(from = 0, to = 4, by = 0.5), Inf)), 'RdBu')))
