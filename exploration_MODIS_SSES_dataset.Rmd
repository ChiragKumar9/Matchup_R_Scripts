...
ggplot2::ggplot(data = orig, aes(x = buoy.sst, y = SST.resid.SMB,
...
p <- rasterVis::levelplot(grid5deg, margin = FALSE,
  ...
  col.regions = rev(RColorBrewer::brewer.pal(length(c(-Inf, seq(from = -0.4, to = 0.4, by = 0.1), Inf)), 'RdBu')))
...
p <- rasterVis::levelplot(grid5deg, margin = FALSE,
  at = c(-Inf, seq(from = 0, to = 2, by = 0.2), Inf), # For IQR
  #at = c(1, 500, 1000, 2000, 5000, 10000, 15000, Inf), # For N
  col.regions = rev(RColorBrewer::brewer.pal(length(c(-Inf, seq(from = 0, to = 2, by = 0.2), Inf)), 'RdBu')))
...
```{r}
ggplot2::ggplot(data = orig, aes(x = band.diff, y = SST.resid.SMB,
  fill = cut(..count.., c(0, 20, 100, 500, 1000, 1500, 2000, 4000, Inf)))) +
  geom_bin2d(bins = 100) + 
  scale_fill_hue('count') +
  geom_abline(slope = 0, intercept = 0, col = 'black')
...
cutoff_val <- 5
qqq <- data.frame(condition = ifelse(orig$band.diff > cutoff_val, paste('band.diff greater than', cutoff_val), paste('band.diff less than', cutoff_val)), resid = orig$SST.resid.SMB)

ggplot2::ggplot(data = qqq, aes(x = resid, fill = condition)) +
  geom_density(alpha = .4, color = NA)

overlap_coef <- overlap(qqq[qqq$condition == paste('band.diff greater than', cutoff_val), 'resid'], qqq[qqq$condition == paste('band.diff less than', cutoff_val), 'resid'])
...
cutoff_val <- 10
qqq <- data.frame(condition = ifelse(orig$x2 > cutoff_val, paste('x2 greater than', cutoff_val), paste('x2 less than', cutoff_val)), resid = orig$SST.resid.SMB)
