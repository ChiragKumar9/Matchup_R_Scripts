...
qqq <- data.frame(condition_band_diff = ifelse(orig$band.diff > cutoff_val, ifelse(orig$band.diff > 3, 2, 1), 0),
                  condition_satz = cut(abs(orig$satz), c(0, 10, 20, 30, 40, 50, 60, max(orig$satz) + .001), include.lowest = TRUE),
                  resid = orig$SST.resid.SMB + 0.17)

qqq$condition_band_diff <- factor(qqq$condition, levels = c(0, 1, 2), labels = c('(-Inf, 0]', '(0, 2]', '(2, Inf)'))

qqq$total <- paste0('band_diff ', as.character(qqq$condition_band_diff), ' and satz ', as.character(condition$satz))

ggplot2::ggplot(data = qqq, aes(x = resid, fill = condition_total)) +
  geom_density(alpha = .4, color = NA)
