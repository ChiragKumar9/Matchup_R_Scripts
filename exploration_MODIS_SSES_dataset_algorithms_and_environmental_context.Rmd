...
tt1 <- dplyr::tbl_df(orig) %>%
  dplyr::filter(qsst == 0)

ccc = RColorBrewer::brewer.pal(n = 8, 'YlOrRd')
ccc <- ccc[3:8]

temp_context <- cut(tt1$buoy.sst, c(-Inf, 10, 25, Inf))
temp_deficit <- tt1$buoy.sst - tt1$x1

uuu <- data.frame(temp_context = as.factor(temp_context), temp_deficit = temp_deficit, band.diff = tt1$band.diff)
