...
# Is this chunk necessary? More of just loading AQUA for info
...
# See what AMSR2 SSTs are like after converting -999 to NA, also see
# that # of -999 = # of NA so conversion worked
summary(VIIRS$amsr2.val)
...
ggplot(data = VIIRS, mapping = aes(x = amsr2.val, y = cen.sst)) +
  stat_bin2d(binwidth = c(0.2, 0.1)) +
  scale_fill_gradientn(colours = ccc) +
  geom_abline(intercept = 0, slope = 1, color = 'steelblue') +
  lims(x = c(-2, 32), y = c(-2,32)) +
  ggplot2::labs(title = "VIIRS and AMSR-2 SSTs", x = 'AMSR-2 SST', y = 'VIIRS SST') +
  ggplot2::ggsave(filename = 'VIIRS_and_AMSR-2_SSTs.ps', device = 'ps',
       width = 8, height = 6, units = 'in') +
  theme(text = element_text(size = 16))
