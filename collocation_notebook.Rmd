...
After applying the various filters we are left with `r nrow(orig_j)` matchups. These matchups now have enough spatial and temporal coincidence for us to apply the collocation techniques.

## Spatial distribution of collocated matchups

Here we explore the geospatial distribution of the final matchups we will use for collocation. We show that collocated matchups exist all over the world with enough incidence in different regions for us to interpret the results of a global collocation experiment without bias.
...
This is the standard Mercator projection of a map with hexagonal bins overlayed showing the relative density of points per bin.
...
In order to interpret the global collocation properly, we must understand the temporal distribution of matchups. Algorithm changes will effect the accuracy of SST retrievals and SST accuracy is dependent on seasonal atmospheres (hence the recomputation of algorithms coefficients on a seasonal bases). We show that there are more matchups during the summer months in North America than the winter months. This finding allows us to interpret the results of the global collocation with the appropriate understanding that the global standard deviation of random errors will be biased towards the standard deviation of errors in summer months.
...
A key assumption in collocation is that the random errors must be uncorrelated. Because MODIS and VIIRS are two very similar instruments that use similar retrieval techniques and view extraordinarily similar atmospheres (see high temporal and spatial similarity explored above), we hypothesize that MODIS and VIIRS retrieval random errors are highly correlated. We attempt to explore the validity of no correlation assumption within our data. We start by exploring the correlation between retrievals in each of the three sensors.
...
The correlation between SST sources is extraordinarily high. However, this still does not tell us what the correlation between random errors in retrievals may be. We attempt to estimate this parameter by finding the correlation between MODIS - buoy SSTs and VIIRS - buoy SSTs.

## Correlation between MODIS and VIIRS Errors
```{r aqua_viirs_errors}
...
Because of the assumed validity of buoy SSTs, we can estimate the random error in a retrieval by subtracting the buoy retrieval value from the satellite retrieval. We show that the correlation between MODIS and VIIRS errors is extraordinarily high, almost 0.7, for two sources whose errors should be uncorrelated for collocation.

# Error estimation using triple collocation

## Global experiment: all matchups in collocated DB (CDB)

We explore the collocation-outputed values of standard deviation in a global experiment, knowing that the random error in MODIS and VIIRS retrievals is highly correlated. We notice that this value does not agree with previous experiments to quantify the random error.
...
We explore the quantity of misprediction in collocation standard deviation when errors are known to be correlated. We create three sets of random errors and systematically change the correlation in these sets. We then plot the ratio of predicted standard deviation to the correct standard deviation with the varying values of correlation. Note that we keep the standard deviation of the random error the same throughout the experiments. However, the standard deviations we use is based on previous analysis of possible standard deviations of random errors in MODIS and VIIRST SSTs.
...
By having an estimation of the correlation of random errors between MODIS and VIIRS (0.7) and knowledge of how correlation effects collocation-estimated standard deviations, we can compute standard deviations on our correlated dataset and correct by dividing by the factor of predicted standard deviation to actual standard deviation determined from our above numerical analysis
