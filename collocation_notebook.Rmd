...
if (Sys.info()["sysname"] == 'Windows') {
  ...
  win.file <- 'VIIRS_Suomi_NPP_MIA_L2GEN_ALL_Class_6.4.1_ao_2016_11_25_with_ancillary.Rdata'
  ...
  # Name of .Rdata file (update as needed)
  linux.file <- 'VIIRS_Suomi_NPP_MIA_L2GEN_ALL_Class_6.4.1_ao_2016_11_25_with_ancillary.Rdata'
...
VIIRS <- VIIRS_Suomi_NPP_MIA_L2GEN_ALL_Class_6.4.1_ao_2016_11_25
rm(VIIRS_Suomi_NPP_MIA_L2GEN_ALL_Class_6.4.1_ao_2016_11_25)
...
if (Sys.info()["sysname"] == 'Windows') {
  ...
  # Name of .Rdata file (update as needed)
  win.file <- 'MODIS_Aqua_GSFC_ALL_Class_6.4.1_ao_2016_11_25_with_ancillary.Rdata'
  viirs.file <- paste0(win.dir, win.file)
  if (!file.exists(viirs.file)) {
    stop('Input file does not exist')
  } else {
  ...
  linux.dir <- '~/Projects/Matchup_R_Scripts/Results/objects/'
  # Name of .Rdata file (update as needed)
  linux.file <- 'MODIS_Aqua_GSFC_ALL_Class_6.4.1_ao_2016_11_25_with_ancillary.Rdata'
...
AQUA <- MODIS_Aqua_GSFC_ALL_Class_6.4.1_ao_2016_11_25
rm(MODIS_Aqua_GSFC_ALL_Class_6.4.1_ao_2016_11_25)
...
mp <- mp + 
  ggplot2::stat_bin2d(data = orig_filtered,
    aes(x = buoy.lon.aqu, # Use hexagonal binning command and give x and y input
    y = buoy.lat.aqu,
    fill = cut(..count.., breaks = quantile(..count.., probs = c(seq(0, 1, 0.2))))), # Divides matchups per bin into discrete chunks and colors likewise
    binwidth = c(10, 10)) +
...
fit_viirs__buoy_sst <- lm(cen.sst.vrs ~ buoy.sst.vrs, data = orig_filtered)

# Compute robust standard deviation

RSD_viirs_buoy <- IQR(fit_viirs__buoy_sst$residuals) / 1.348
```
...
calculate_bias <- function(source1, source2, source3) {
  ...
  return(biases)
}   # End of calculate_bias


calculate_variance <- function(source1, source2, source3) {
  if (!is.null(nrow(source1)) |
    !is.null(nrow(source2)) |
    !is.null(nrow(source3)) |
    length(source1) <= 1 |
  ...
  biases <- calculate_bias(source1, source2, source3)
...
# Make sure the last argument to calculate_variance corresponds to the buoy SSTs
 ...
 variances_global <- calculate_variance(orig_filtered$cen.sst.vrs,
  orig_filtered$cen.sst.aqu,
  orig_filtered$buoy.sst.vrs)

  biases_global <- calculate_bias(orig_filtered$cen.sst.vrs,
    orig_filtered$cen.sst.aqu,
    orig_filtered$buoy.sst.vrs)
